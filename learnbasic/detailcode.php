...
<body>
    ...
    echo '<div class="header">
			...
			while ($rowcopy = mysqli_fetch_array($resultcopy)) {
				...
				$FrameworkCodename = $rowcopy['framework_codename'];
				$FrameworkCodetype = $rowcopy['framework_codetype'];
                $Detail_m_lastupdate = $rowcopy['m_detail_lastupdate'];
			echo '<div>
				'.$Detail_m_id.' - '.$FrameworkCodename.' - '.$FrameworkCodetype.' <br>
				'.$Detail_m_subject.' <br>
                '.$Detail_m_name.' <br>
			</div>';
			echo '<pre>
			<code class="language-html" id="example-head">
				'.htmlspecialchars($Detail_m_textcode).'
			</code>
			</pre>';
			}
